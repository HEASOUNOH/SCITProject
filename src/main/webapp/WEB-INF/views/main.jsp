						...
						<div class="row">
								<div class="card card-tasks">
									<div class="card-header ">
										<h4 class="card-title" style="color:Orange; font-size: 20px;" >プログレス</h4>
										<p class="card-category">スケジュール確認</p>
									</div>
									<div class="card-body ">
									 <table class="table table-hover" border="1" style="border-style: none">
											<tr id="Querie">
												<th><img src="assets/img/checkimg.PNG" width="20" height="15"></th>
												<th>スケジュール名前</th>
												<th>ストーリー</th>
												<th>パーセント</th>
												<th>修整／削除</th>
											...
											<c:if test="${empty replylist}">
											  <tbody id="result">
													<tr>
														<td colspan="5">-　今日が含まれたスケジュールがありません。</td>
													</tr>
											</c:if>
									  </tbody>
								</table>
 ...
 $(function(){
	...
	function output(resp){
		...
		$.each(resp.replylist, function(index, item) {
			...
			if(item.checked == '1'){
				result += '<td><div class="selected"><input class="check" class="test" type="checkbox" data-cno="' + item.schNum + '" value="' + item.checked + ' "checked="checked""/></div></td>';//<사진> 
				console.log("체크인");
			}else{
				console.log("체크아웃");
				result += '<td><div class="selected"><input class="check" class="test" type="checkbox" data-cno="' + item.schNum + '" value="' + item.checked + '"/></div></td>';//<사진> 
			}
			...
			result += '<td><div class="num' + index + '"><div id="percentnum"></div></div></td>';//<퍼센트>
			result += '<td>'
